...
| Command | Description | Request fields | Response fields | Frontend usage | Backend handler |
|---------|-------------|----------------|----------------|----------------|----------------|
| `travel.providerStatus` | Read TREK provider runtime/auth/mirror/update readiness for the active vault | — | `TravelProviderStatus` (`runtimeMode`, `sidecar.*`, warnings, additive bundled-runtime provenance: `runtimeSourceType`, `bundledRuntimeVersion`, `bundledRuntimeBuildManifestPath`, `updatePolicy`) | Travel provider gate/status card (Settings + Travel route shell) | `travel_provider_status` |
...
| `travel.providerCheckUpdates` | In packaged builds, compare the active runtime against the Cortex-prepared pinned TREK bundle; in explicit dev-override mode, upstream release probing may still be enabled for maintainer validation | — | `TravelProviderUpdateCheckResult` (`candidateVersion`, `compatible`, `compatibilityNotes[]`, candidate asset metadata, additive policy/provenance fields such as `updateSourceType`, `candidateSourceType`, `candidatePinned`, bundled-runtime/build-manifest context) | Travel provider update panel (preflight check + gate visibility) | `travel_provider_check_updates` |
| `travel.providerApplyUpdate` | Activate the validated packaged pinned TREK runtime candidate (or explicit dev-override candidate) and rollback to the prior active version on post-apply runtime health failure | `targetVersion` | `TravelProviderApplyUpdateResult` (`targetVersion`, `status`, `applied`, `rolledBack`, `previousVersion`, `activeVersion`, warnings, additive active-version provenance) | Travel provider update panel (activate candidate + rollback feedback) | `travel_provider_apply_update` |
...
| `integrations.bulkConvertGoogleEventsToTasks` | Explicitly convert matching editable Google events within a calendar/date window into Cortex tasks | `{ calendarIds: string[], startDate, endDate }` | `{ convertedCount, alreadyConvertedCount, skippedCount, taskIds }` | Week planner bulk import modal | `integrations_bulk_convert_google_events_to_tasks` |
