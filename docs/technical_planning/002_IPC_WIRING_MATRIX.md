...
| `finance.ynabStatus` | Read local YNAB connection/sync status. `FinanceYnabStatus` may include `connectionIssue?` when a stored PAT exists but cannot be decrypted; clients must treat that as disconnected and prompt reconnect. | — | `FinanceYnabStatus` | Settings Integrations (YNAB) + Finance YNAB gate/status card | `finance_ynab_status` |
