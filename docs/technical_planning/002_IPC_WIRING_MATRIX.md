...
| `finance.ynabStatus` | Read local YNAB connection/sync status. `FinanceYnabStatus` may include `connectionIssue?` when a stored PAT exists but cannot be decrypted; clients must treat that as disconnected, suppress normal sync affordances, and prompt reconnect via Settings → Integrations. | — | `FinanceYnabStatus` | Settings Integrations (YNAB) + Finance YNAB gate/status card | `finance_ynab_status` |
...
| `finance.ynabSync` | Full/delta YNAB view-only sync to local `ynab_*` pages + canonical `Finance/YNAB/current` CSV mirror + analytics refresh. `warnings[]` may report historical month-detail backfill or legacy namespace cleanup. | `budgetId?`, `mode?` (`auto\\|full\\|delta`), `writeCsv?`, `reanalyze?` | `FinanceYnabSyncResult` | Finance YNAB sync controls | `finance_ynab_sync` |
...
| `finance.ynabGetAnalytics` | Compute/read local YNAB whole-budget + tracked-category metrics. `categoryBreakdown[]` is additive with `carriedFromLastMonthMilliunits` and `availableBeforeActivityMilliunits` so clients can render assigned-only values separately from carryover-aware spending bars; YNAB `balance` remains the source for current `$ Available`, and payment/transfer/inflow buckets are excluded from spending-breakdown consumers. | `monthKey?`, `budgetId?` | `FinanceYnabAnalytics` | Finance dashboard charts + category metrics | `finance_ynab_get_analytics` |
...
| `integrations.syncStatus` | Read worker-backed Google sync/runtime status, queue depth, last run timestamps, and index backlog snapshot | — | `IntegrationSyncStatus` (`status`, `runId?`, `phase?`, `calendarId?`, `calendarName?`, `processed?`, `total?`, `message?`, `queuedRequests`, `active`, `lastRequestedAt?`, `lastStartedAt?`, `lastFinishedAt?`, `lastSyncedAt?`, `lastError?`, `queuedIndexJobs`, `indexedJobs`, `failedIndexJobs`, `skippedIndexJobs`, `totalIndexJobs`) | Calendar sync queue, Settings Integrations status | `integrations_sync_status` |
| `integrations.triggerSync` | Enqueue or wake a Google sync wave and emit progress telemetry from the background worker lane | — | `boolean` | Settings Integrations, background sync queue | `integrations_trigger_sync` |
...
| `obsidian.syncNow` | Enqueue or wake an immediate linked-vault sync run on the existing runtime lane | `request: { link_id }` | `SyncRun` (`status: "queued"\|"running"\|"done"\|"failed"`, `phase` may be `"queued"` before runtime pickup) | Settings Integrations sync controls | `obsidian_sync_now` |
| `obsidian.syncStatus` | Read linked-vault sync status, recent runs, queue counts, and recent failed files for a link, including queued manual-run overlays before the runtime starts work | `request: { link_id, limit? }` | `SyncStatus` (`failedFiles?` additive list of per-file failures scoped to the link) | Settings Integrations status/progress panel | `obsidian_sync_status` |
